#!/usr/bin/env python3
"""Migrate WordPress WXR export into Astro content collections for interieurwonenplaza.nl."""
...
XML_PATH = Path(
    os.environ.get(
        "WXR_PATH",
        "/Users/gauravkashyap/Downloads/interieurwonenplazanl.WordPress.2026-06-27.xml",
    )
)
SITE_URL = "https://interieurwonenplaza.nl"
...
DEFAULT_IMAGE = "/images/2023/01/hd-wallpaper-g32e4eb954_1280-1024x682.jpg"
...
def rewrite_urls(content: str, url_map: dict[str, str]) -> str:
    ...
    content = re.sub(
        rf"https?://(?:www\.)?interieurwonenplaza\.nl/wp-content/uploads/[^\s\"'<>]+",
        replace_upload_url,
        content,
    )
    content = re.sub(
...
def extract_first_image(content: str) -> str | None:
    for pattern in [
        ...
        r'src="(https://interieurwonenplaza\.nl/wp-content/uploads/[^"]+)"',
    ]:
        ...
        if match:
            ...
            return upload_to_local(f"{SITE_URL}{src}" if src.startswith("/") else src)
    return None
...
def sanitize_mdx_content(text: str) -> str:
    """Fix patterns that break MDX parsing."""
    text = re.sub(r"<(https?://[^>\s]+)>", r"[\1](\1)", text)
    text = re.sub(r"<(www\.[^>\s]+)>", r"[\1](https://\1)", text)
    text = re.sub(
        r"/electrowarmer-gebruiken-tips-voor-veilig-slim-en-energiezuinig-verwar/\.\.\.",
        "/elektrowarmer-gebruiken-handige-tips-voor-veilig-en-efficient-verwarmen/",
        text,
    )
    text = re.sub(
        r"/electrowarmer-gebruiken-tips-voor-veilig-slim-en-energiezuinig-verwarmen/",
        "/elektrowarmer-gebruiken-handige-tips-voor-veilig-en-efficient-verwarmen/",
        text,
    )
    return text
...
def prepare_mdx_body(html: str) -> str:
    ...
    try:
        ...
        text = re.sub(r"\n{3,}", "\n\n", text).strip()
        return sanitize_mdx_content(text.replace("{", "\\{").replace("}", "\\}"))
    except ImportError:
...
def load_existing_products() -> dict:
    ...
    if products_path.exists():
        for item in json.loads(products_path.read_text(encoding="utf-8")):
            slug = item["slug"]
            products[slug] = {
                "slug": slug,
                "title": item.get("title", slug),
                "description": f"Lees alles over {slug.replace('beste-', '').replace('-', ' ')} op InterieurWonenPlaza.nl.",
...
def write_pages_json(page_items: list[dict], products: dict) -> None:
    ...
    blog_slugs = []
    if (DATA_DIR / "blog-posts.json").exists():
        blog_slugs = [p["slug"] for p in json.loads((DATA_DIR / "blog-posts.json").read_text())]

    existing_slugs = []
    if (DATA_DIR / "slugs.json").exists():
        existing_slugs = json.loads((DATA_DIR / "slugs.json").read_text())

    slugs = sorted(
        set(pages_data.keys())
        | set(blog_slugs)
        | set(existing_slugs)
        | {"over-ons", "contact", "blogs", "blog", "sitemap"}
    )
